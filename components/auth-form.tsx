...
export function AuthForm({ initialTab = 'signin', onSubmit }: AuthFormProps) {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (errorStr.includes('존재하지 않는 계정')) {
          errorMessage =
            '존재하지 않는 계정입니다. 회원가입을 먼저 진행해주세요.';
        } else if (
          errorStr.includes('비밀번호가 일치하지 않습니다') ||
          errorStr.includes('비밀번호가 알맞지 않습니다')
        ) {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        // 오류 메시지 정제
        if (
          errorStr.includes('쿨다운') ||
          errorStr.includes('초 후에 다시 시도')
        ) {
          errorMessage = error.message; // 이미 사용자 친화적인 메시지
        } else if (
          errorStr.includes('이미 가입된 이메일') ||
          ...
          errorMessage =
            '이미 가입된 이메일입니다. 로그인 페이지로 이동합니다.';
          shouldSwitchToLogin = true; // 로그인 탭으로 전환 표시
        } else if (
          errorStr.includes('비밀번호') &&
          (errorStr.includes('최소') || errorStr.includes('길이'))
        ) {
      ...
      <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-md flex items-start gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-sm text-green-700 dark:text-green-400 mt-1">
            <strong>{registeredEmail}</strong> 주소로 인증 이메일을
            발송했습니다. 이메일을 확인하여 가입을 완료해주세요.
          </p>
            ...
            <CardContent className="space-y-4 pt-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  required
                  className={
                    signinError ? 'border-red-300 focus:ring-red-400' : ''
                  }
                />
              </div>
              <div className="space-y-2">
            ...
            <CardContent className="space-y-4 pt-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  required
                  className={
                    signupError ? 'border-red-300 focus:ring-red-400' : ''
                  }
                />
              </div>
              <div className="space-y-2">
